...
getBestBinTh <- function(object, quantiles) {
  ...
  for (quantile in quantiles) {
    gene.thresholds <- sapply(insitu.genes, 
                              function(gene) quantile(
                                as.numeric(object@data[gene, object@data[gene, ] > 0]), 
                                quantile))
  ...
  return (best.quantile)
}


binFun <- function(object, quantiles) {
  insitu.genes <- colnames(object@insitu.matrix)
  best.quantile <- quantiles
